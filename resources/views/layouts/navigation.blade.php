<nav x-data="{ open: false, openNotification: false }" class="fixed w-full z-50 bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            ...
            <div class="flex items-center relative ml-auto mr-6 sm:mr-0">
                <x-dropdown align="right" width="w-96">
                    <x-slot name="trigger">
                            <x-buttons.icon-button-secondary class="border-opacity-0 shadow-none flex" id="button-like-post" data="">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" />
                                </svg>
                                <p class="bg-red-500 text-white text-center text-sm h-5 min-w-[1.25rem] flex justify-center items-center w-auto rounded-full absolute top-0 left-0 p-1">9</p>
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
                            <img  class="w-5 h-5 mr-2 rounded-full mx-auto object-cover" src="{{Auth::user()->profile_image}}" alt="">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('posts', auth()->user()->id)" :active="request()->is('post/'.auth()->user()->id)">
                {{ __('My posts') }}
            </x-responsive-nav-link>
        </div>

        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('connect')" :active="request()->routeIs('connect') || request()->routeIs('search')">
                {{ __('Connect') }}
            </x-responsive-nav-link>
        </div>
